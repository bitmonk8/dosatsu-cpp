...
// Simple replacements for system functionality (for self-contained example)
// Note: In a real preprocessor example, these would normally include system headers

// Simple printf replacement for demonstration  
void simple_printf(const char* format, ...) {
    // Simplified implementation - just to show variadic macros work
    (void)format; // Suppress unused warning
}

// Platform detection without system includes
#ifdef _WIN32
    #define PLATFORM_NAME "Windows"
    #define PLATFORM_SPECIFIC_FUNCTION() /* Windows-specific code would go here */
#elif defined(__linux__)
    #define PLATFORM_NAME "Linux"
    #define PLATFORM_SPECIFIC_FUNCTION() /* Linux-specific code would go here */
#elif defined(__APPLE__)
    #define PLATFORM_NAME "macOS"
    #define PLATFORM_SPECIFIC_FUNCTION() /* macOS-specific code would go here */
#else
    #define PLATFORM_NAME "Unknown"
    #define PLATFORM_SPECIFIC_FUNCTION() /* Generic code would go here */
#endif
...
// Debug/Release configuration
    ...
    #define ASSERT(condition) \
        do { \
            if (!(condition)) { \
                simple_printf("Assertion failed: %s at %s:%d\n", \
                        #condition, __FILE__, __LINE__); \
                /* abort() replaced with infinite loop for self-contained example */ \
                while(1) {} \
            } \
        } while(0)
#else
...
// Note: #pragma once removed as it's not appropriate for main files
...
#define LOG_INFO(...) \
    do { \
        if (DEBUG_ENABLED) { \
            simple_printf("[INFO] " __VA_ARGS__); \
            simple_printf("\n"); \
        } \
    } while(0)
...
#define LOG_ERROR(...) \
    do { \
        simple_printf("[ERROR] " __VA_ARGS__); \
        simple_printf("\n"); \
    } while(0)
...
#define PRINT_1(a) simple_printf("%d\n", a)
#define PRINT_2(a, b) simple_printf("%d %d\n", a, b)
#define PRINT_3(a, b, c) simple_printf("%d %d %d\n", a, b, c)
...
// Count of colors (6 colors defined in COLOR_LIST)
...
// Simple printf replacement for test output
void simple_printf(const char* format, const char* arg) {
    // In a real application, this would output to console/file
    // For this example, we just process the arguments silently
