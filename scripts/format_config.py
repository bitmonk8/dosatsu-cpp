#!/usr/bin/env python3
"""
Format configuration helpers for Dosatsu build system.
...
def run_format_check(files: List[Path], project_root: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            error_msg += "\nRun 'please format' to fix formatting issues"
...
please format --check-only --files $(git diff --cached --name-only --diff-filter=ACM | grep -E "\\.(cpp|h|hpp|cxx|cc)$" | tr '\\n' ' ')
